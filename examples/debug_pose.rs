...
fn debug_pose_message(data: &[u8]) -> Result<()> {
    ...
    // Scan for f64 values that look like reasonable pose coordinates
    ...
    for i in (4..data.len()-7).step_by(4) {
        if i + 8 <= data.len() {
            ...
            if (value >= -10.0 && value <= 10.0) || (value >= -1.0 && value <= 1.0) {
    ...
    // PoseWithCovarianceStamped structure:
    // - Header (already read)
    // - PoseWithCovariance:
    //   - Pose:
    //     - Point position (x, y, z) - 3 f64 values
    //     - Quaternion orientation (x, y, z, w) - 4 f64 values
    //   - covariance[36] - 36 f64 values
    ...
    for test_pos in [pos, pos + 4, pos + 8, pos + 12, pos + 16] {
        if test_pos + 56 <= data.len() { // 7 f64 values = 56 bytes
            ...
            if pos_reasonable && ori_reasonable {
...
fn main() -> Result<()> {
    ...
    let target_connections: Vec<_> = reader.connections()
        .iter()
        .filter(|conn| conn.topic == "/vio/scaled_absolute_pwc")
        .cloned()
        .collect();
